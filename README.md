...
2. Connect the public Matters plugin to Codex, or configure another compatible
   AI-agent/MCP host.
3. Grant the source scopes that the agent is allowed to register and analyze.
...
6. Optionally create a daily Codex scheduled task that calls the same planned
   maintenance path. Recurrence is never enabled silently; it requires explicit
   user opt-in and remains bound to the granted scopes.
...
6. 用户可以选择建立每日 Codex 计划任务，让它调用同一条计划维护路径。软件不会
   静默开启每日任务；必须由用户明确同意，而且任务始终受已授权范围约束。
