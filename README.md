...
<p align="center">
  <strong>Turn scattered authorized clues into a clear map of the big situations shaping a person's life.</strong>
</p>
<!-- README HERO END -->
...
Matters is a local-first application that uses clues from user-authorized
files, mail, projects, and other information sources to reconstruct a person's
**larger real-life situations**. It is not a universal memory archive and it
does not turn every message or small action into a card. The small records are
evidence for a smaller set of human-scale Matters—such as a trip, a job search,
a software project, a household decision, or another continuing situation.

For each Matter, the system builds an explicit model of what the situation is,
when it began, what stage it has reached, which important parts are planned,
active, or completed, who and what are related, and which questions remain
open. This gives both the person and their AI a usable overview without asking
the person to repeatedly reconstruct the same background from scattered files.
...
Matters is designed to work with **Codex or another compatible AI-agent host**
(for example, a properly configured Claude Code environment) that can load the
Matters skill, connect to its MCP gateway, and run bounded
maintenance work. Matters ships the skill contracts, local model store,
maintenance interfaces, and desktop browser; the AI host supplies the
reasoning that reads authorized content, relates clues, proposes models, and
keeps them current. The release does not hide a separate cloud model or require
an OpenAI API key inside the application.
...
The two entrypoints have different jobs. The desktop application **displays and
explores models that already exist**; it is not a replacement for the AI agent
that creates and refreshes those models. The bundled eleven-skill pack tells the
agent how to perform governed maintenance, but a compatible host such as Codex
must actually invoke those skills and MCP operations.
...
For a useful first run, complete the whole chain rather than opening only the
desktop executable:

1. Install Matters and keep <code>MATTERS_HOME</code> outside the public source
   checkout.
2. Connect the public Matters plugin to Codex, or configure another compatible
   AI-agent/MCP host.
3. During installation, tell the installing AI which folders, mailboxes, or
   other information sources it may register and analyze. Matters ships no
   hard-coded personal scope and the AI must not invent one.
4. Ask the agent to run an initial bounded maintenance cycle. It registers
   sources in place, excludes unsuitable content, builds the first Matter
   models, and records any gaps.
5. Open the desktop application to inspect the resulting cards, hierarchy,
   timelines, sources, and coverage state.
6. During installation, the installing AI automatically creates or repairs
   exactly one daily maintenance schedule that calls the same bounded service
   path. It chooses a sensible low-activity local time from available context,
   or uses 21:00 local when no better signal exists. The user does not create
   this task manually; if the AI host cannot manage schedules, setup must stop
   with that blocker visible instead of claiming completion.

The complete AI-owned setup contract is in
[<code>AI installation guide</code>](./plugins/matters/skills/matters/references/installation.md).
The schedule never widens the source scopes granted in step 3 and never owns
release, Git, installation-currentness, or final full-test verification.

If <code>Matters.exe</code> is opened before an AI maintenance run has produced
any models in the selected private runtime, the window can be healthy and still
show an empty catalog. In that state it is the UI shell over an empty model
store, not evidence that the authorized information has already been read.
...
The plugin teaches Codex how to inspect the current information map, leave
typed observations and corrections, and invoke bounded or planned maintenance.
Other AI-agent hosts are compatible only when they can load an equivalent
skill contract, connect to this MCP server, and own the scheduled execution.
...
local-first and provides no hosted synchronization service. When the user asks
an AI to install and use Matters, that installing AI owns the daily-maintenance
setup; the user separately supplies the information-source scope.
...
Matters 是一款本地优先的软件。它根据用户授权的文件、邮件、项目和其他信息来源中的
线索，重建一个人正在经历的**大事情和整体处境**。它不是要记住每一条聊天或每一个
小动作，也不会把每封邮件、每个文件都变成一张卡片。小记录只是证据，最后应当汇入
少量人类能理解的大事项，例如一次旅行、一次求职、一个软件项目、一项住宅决策，或
另一件持续发展的重要事情。

对于每个大事项，系统会整理它是什么、何时开始、目前走到哪个阶段、哪些重要部分是
计划中、进行中或已完成、涉及哪些人物和资料，以及还有什么没有闭合。这样，人和 AI
都能看到一个可用的整体概况，不需要每次再从散落文件中重新解释全部背景。

它的结果不是一个看不见过程的黑盒摘要。重要结论仍然连接着原始位置、时间依据、
不确定性、推断状态、纠正历史和当前新鲜度，因此可以审计、追踪、检查，并在新线索
出现后重新判断。

Matters 的完整使用方式依赖 **Codex 或其他兼容的 AI 智能体宿主**（例如经过相应配置
的 Claude Code 环境）：宿主需要能够加载 Matters 技能、连接 MCP 入口，并执行有边界
的维护任务。Matters 提供技能契约、
本地模型库、维护接口和桌面浏览器；AI 宿主负责真正读取已授权内容、关联线索、提出
模型并持续更新。软件内部没有隐藏的另一套云端模型，也不要求用户在 Matters 中配置
OpenAI API 密钥。
...
两个入口承担的工作并不一样：桌面程序负责**显示和浏览已经存在的模型**，它不能
替代负责建立和更新模型的 AI 智能体。随软件打包的 11 项技能规定了受治理的维护方法，
但仍然需要 Codex 或兼容宿主实际调用这些技能和 MCP 操作。
...
第一次使用时，需要完成整条链路，而不是只打开桌面 EXE：

1. 安装 Matters，并把 <code>MATTERS_HOME</code> 放在公开源码仓库之外。
2. 把公开 Matters 插件连接到 Codex，或者配置其他兼容的 AI 智能体/MCP 宿主。
3. 安装时，把允许登记和分析的文件夹、邮箱或其他信息来源范围告诉安装 AI。Matters
   不会把任何个人调查范围写死在软件里，AI 也不能自行猜测或扩大范围。
4. 让 AI 执行第一次有边界的维护：原位登记来源、排除不适合分析的内容、建立首批
   事项模型，并记录尚未完成的缺口。
5. 打开桌面程序，查看生成的卡片、层级、时间线、来源和覆盖状态。
6. 安装 AI 自动创建或修复唯一一条每日维护计划，让它调用同一条有边界的维护路径。
   AI 会根据可用上下文选择本地低活动时间；没有更好依据时默认使用本地时间 21:00。
   用户不需要手工创建这条任务；如果 AI 宿主没有计划任务能力，安装必须明确报告
   这个阻塞，不能假装已经完成。

完整的 AI 自动安装合同见
[<code>AI 安装指南</code>](./plugins/matters/skills/matters/references/installation.md)。
每日任务始终受第 3 步授权范围约束，也不能负责发布、Git、安装一致性或最终全量测试。

如果在 AI 还没有向所选私有运行目录写入任何模型之前就打开
<code>Matters.exe</code>，窗口本身可以完全正常，但目录会是空的。此时看到的只是
连接空模型库的 UI 壳，并不代表已授权信息已经被读取或建模。
...
这个插件会告诉 Codex 怎样读取当前信息地图、留下有类型的观察与纠正，以及启动
有边界的普通维护或计划维护。其他 AI 智能体宿主只有在能够加载等价技能契约、连接
这个 MCP 服务并负责计划任务执行时，才属于兼容使用方式。
...
Matters 是本地优先软件，不提供托管式云同步。用户把“安装并使用 Matters”交给 AI
后，安装 AI 负责建立每日维护；读取哪些来源仍然只由用户授权范围决定。
